...
function getRandomHand(pokemon) {
  const shuffled = pokemon.sort(() => 0.5 - Math.random());
  const hand1 = shuffled.slice(0, shuffled.length / 2);
  const hand2 = shuffled.slice(shuffled.length / 2);
  return [hand1, hand2];
}

function Pokegame({ pokemon = defaultPokemon }) {
  const [hand1, hand2] = getRandomHand(pokemon);
